; Listing generated by Microsoft (R) Optimizing Compiler Version 17.00.61030.0 
	...
	TITLE	C:\Users\Simon\Documents\Visual Studio 2012\Projects\Stereoscopic\Model.cpp
	.686P
	.XMM
	include listing.inc
	.model	flat
...
EXTRN	?g_pd3dDevice@@3PAUID3D10Device@@A:DWORD	; g_pd3dDevice
...
_TEXT	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File c:\users\simon\documents\visual studio 2012\projects\stereoscopic\import\cimportxfile.h
...
text$yd	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File c:\users\simon\documents\visual studio 2012\projects\stereoscopic\import\common\msdefines.h
...
text$yc	ENDS
; Function compile flags: /Odtp /RTCsu /ZI
; File c:\users\simon\documents\visual studio 2012\projects\stereoscopic\model.cpp
...
; 259  : 	// Don't render if no geometry
; 260  : 	if (!m_HasGeometry)
...
; 263  : 	}
; 264  : 
; 265  : 	// Select vertex and index buffer - assuming all data will be as triangle lists
; 266  : 	UINT offset = 0;

	mov	DWORD PTR _offset$[ebp], 0

; 267  : 	g_pd3dDevice->IASetVertexBuffers( 0, 1, &m_VertexBuffer, &m_VertexSize, &offset );
...
; 268  : 	g_pd3dDevice->IASetInputLayout( m_VertexLayout );
...
; 269  : 	g_pd3dDevice->IASetIndexBuffer( m_IndexBuffer, DXGI_FORMAT_R16_UINT, 0 );
...
; 270  : 	g_pd3dDevice->IASetPrimitiveTopology( D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST );
...
; 271  : 
; 272  : 	// Render the model. All the data and shader variables are prepared, now select the technique to use and draw.
; 273  : 	// The loop is for advanced techniques that need multiple passes - we will only use techniques with one pass
; 274  : 	D3D10_TECHNIQUE_DESC techDesc;
; 275  : 	technique->GetDesc( &techDesc );
...
; 276  : 	for( UINT p = 0; p < techDesc.Passes; ++p )
...
$LN3@Render:
	...
	jae	SHORT $LN5@Render

; 277  : 	{
; 278  : 		technique->GetPassByIndex( p )->Apply( 0 );
...
; 279  : 		g_pd3dDevice->DrawIndexed( m_NumIndices, 0, 0 );
...
; 216  : 	{
; 217  : 		m_Rotation.x += RotSpeed * frameTime;
...
; 224  : 	{
; 225  : 		m_Rotation.y += RotSpeed * frameTime;
...
; 232  : 	{
; 233  : 		m_Rotation.z += RotSpeed * frameTime;
...
; 238  : 	}
; 239  : 
; 240  : 	// Local Z movement - move in the direction of the Z axis, get axis from world matrix
; 241  : 	if (KeyHeld( moveForward ))
...
; 242  : 	{
; 243  : 		m_Position.x += m_WorldMatrix._31 * MoveSpeed * frameTime;
...
; 244  : 		m_Position.y += m_WorldMatrix._32 * MoveSpeed * frameTime;
...
; 245  : 		m_Position.z += m_WorldMatrix._33 * MoveSpeed * frameTime;
...
; 248  : 	{
; 249  : 		m_Position.x -= m_WorldMatrix._31 * MoveSpeed * frameTime;
...
; 250  : 		m_Position.y -= m_WorldMatrix._32 * MoveSpeed * frameTime;
...
; 251  : 		m_Position.z -= m_WorldMatrix._33 * MoveSpeed * frameTime;
...
; 197  : 	// Make a matrix for position and scaling, and one each for X, Y & Z rotations
; 198  : 	D3DXMATRIX matrixXRot, matrixYRot, matrixZRot, matrixTranslation, matrixScaling;
...
; 199  : 	D3DXMatrixRotationX( &matrixXRot, m_Rotation.x );
...
; 200  : 	D3DXMatrixRotationY( &matrixYRot, m_Rotation.y );
...
; 201  : 	D3DXMatrixRotationZ( &matrixZRot, m_Rotation.z );
...
; 202  : 	D3DXMatrixTranslation( &matrixTranslation, m_Position.x, m_Position.y, m_Position.z );
...
; 203  : 	D3DXMatrixScaling( &matrixScaling, m_Scale.x, m_Scale.y, m_Scale.z );
...
; 204  : 
; 205  : 	// Multiply above matrices together to get the effect of them all combined - this makes the world matrix for the rendering pipeline
; 206  : 	// Order of multiplication is important, get slightly different control mechanism depending on order
; 207  : 	m_WorldMatrix = matrixScaling * matrixZRot * matrixXRot * matrixYRot * matrixTranslation;
...
; 65   : 	// Release any existing geometry in this object
; 66   : 	ReleaseResources();
...
; 67   : 
; 68   : 	// Use CImportXFile class (from another application) to load the given file. The import code is wrapped in the namespace 'gen'
; 69   : 	gen::CImportXFile mesh;

	lea	ecx, DWORD PTR _mesh$[ebp]
	call	??0CImportXFile@gen@@QAE@XZ		; gen::CImportXFile::CImportXFile
	mov	DWORD PTR __$EHRec$[ebp+8], 0

; 70   : 	if (mesh.ImportFile( fileName.c_str() ) != gen::kSuccess)
...
; 73   : 	}
; 74   : 
; 75   : 	// Get first sub-mesh from loaded file
; 76   : 	gen::SSubMesh subMesh;
; 77   : 	if (mesh.GetSubMesh( 0, &subMesh, tangents ) != gen::kSuccess)
...
; 81   : 
; 82   : 
; 83   : 	// Create vertex element list & layout. We need a vertex layout to say what data we have per vertex in this model (e.g. position, normal, uv, etc.)
; 84   : 	// In previous projects the element list was a manually typed in array as we knew what data we would provide. However, as we can load models with
; 85   : 	// different vertex data this time we need flexible code. The array is built up one element at a time: ask the import class if it loaded normals, 
; 86   : 	// if so then add a normal line to the array, then ask if it loaded UVS...etc
; 87   : 	unsigned int numElts = 0;
...
; 89   : 	// Position is always required
; 90   : 	m_VertexElts[numElts].SemanticName = "POSITION";   // Semantic in HLSL (what is this data for)
...
; 91   : 	m_VertexElts[numElts].SemanticIndex = 0;           // Index to add to semantic (a count for this kind of data, when using multiple of the same type, e.g. TEXCOORD0, TEXCOORD1)
...
; 92   : 	m_VertexElts[numElts].Format = DXGI_FORMAT_R32G32B32_FLOAT; // Type of data - this one will be a float3 in the shader. Most data communicated as though it were colours
...
; 93   : 	m_VertexElts[numElts].AlignedByteOffset = offset;  // Offset of element from start of vertex data (e.g. if we have position (float3), uv (float2) then normal, the normal's offset is 5 floats = 5*4 = 20)
...
; 94   : 	m_VertexElts[numElts].InputSlot = 0;               // For when using multiple vertex buffers (e.g. instancing - an advanced topic)
...
; 95   : 	m_VertexElts[numElts].InputSlotClass = D3D10_INPUT_PER_VERTEX_DATA; // Use this value for most cases (only changed for instancing)
...
; 96   : 	m_VertexElts[numElts].InstanceDataStepRate = 0;                     // --"--
...
; 99   : 	// Repeat for each kind of vertex data
; 100  : 	if (subMesh.hasNormals)
...
; 101  : 	{
; 102  : 		m_VertexElts[numElts].SemanticName = "NORMAL";
...
; 103  : 		m_VertexElts[numElts].SemanticIndex = 0;
...
; 104  : 		m_VertexElts[numElts].Format = DXGI_FORMAT_R32G32B32_FLOAT;
...
; 105  : 		m_VertexElts[numElts].AlignedByteOffset = offset;
...
; 106  : 		m_VertexElts[numElts].InputSlot = 0;
...
; 107  : 		m_VertexElts[numElts].InputSlotClass = D3D10_INPUT_PER_VERTEX_DATA;
...
; 108  : 		m_VertexElts[numElts].InstanceDataStepRate = 0;
...
; 113  : 	{
; 114  : 		m_VertexElts[numElts].SemanticName = "TANGENT";
...
; 115  : 		m_VertexElts[numElts].SemanticIndex = 0;
...
; 116  : 		m_VertexElts[numElts].Format = DXGI_FORMAT_R32G32B32_FLOAT;
...
; 117  : 		m_VertexElts[numElts].AlignedByteOffset = offset;
...
; 118  : 		m_VertexElts[numElts].InputSlot = 0;
...
; 119  : 		m_VertexElts[numElts].InputSlotClass = D3D10_INPUT_PER_VERTEX_DATA;
...
; 120  : 		m_VertexElts[numElts].InstanceDataStepRate = 0;
...
; 123  : 	}
; 124  : 	if (subMesh.hasTextureCoords)
...
; 125  : 	{
; 126  : 		m_VertexElts[numElts].SemanticName = "TEXCOORD";
...
; 127  : 		m_VertexElts[numElts].SemanticIndex = 0;
...
; 128  : 		m_VertexElts[numElts].Format = DXGI_FORMAT_R32G32_FLOAT;
...
; 129  : 		m_VertexElts[numElts].AlignedByteOffset = offset;
...
; 130  : 		m_VertexElts[numElts].InputSlot = 0;
...
; 131  : 		m_VertexElts[numElts].InputSlotClass = D3D10_INPUT_PER_VERTEX_DATA;
...
; 132  : 		m_VertexElts[numElts].InstanceDataStepRate = 0;
...
; 135  : 	}
; 136  : 	if (subMesh.hasVertexColours)
...
; 137  : 	{
; 138  : 		m_VertexElts[numElts].SemanticName = "COLOR";
...
; 139  : 		m_VertexElts[numElts].SemanticIndex = 0;
...
; 140  : 		m_VertexElts[numElts].Format = DXGI_FORMAT_R8G8B8A8_UNORM; // A RGBA colour with 1 byte (0-255) per component
...
; 141  : 		m_VertexElts[numElts].AlignedByteOffset = offset;
...
; 142  : 		m_VertexElts[numElts].InputSlot = 0;
...
; 143  : 		m_VertexElts[numElts].InputSlotClass = D3D10_INPUT_PER_VERTEX_DATA;
...
; 144  : 		m_VertexElts[numElts].InstanceDataStepRate = 0;
...
; 149  : 
; 150  : 	// Given the vertex element list, pass it to DirectX to create a vertex layout. We also need to pass an example of a technique that will
; 151  : 	// render this model. We will only be able to render this model with techniques that have the same vertex input as the example we use here
; 152  : 	D3D10_PASS_DESC PassDesc;
; 153  : 	exampleTechnique->GetPassByIndex( 0 )->GetDesc( &PassDesc );
...
; 154  : 	g_pd3dDevice->CreateInputLayout( m_VertexElts, numElts, PassDesc.pIAInputSignature, PassDesc.IAInputSignatureSize, &m_VertexLayout );
...
; 155  : 
; 156  : 
; 157  : 	// Create the vertex buffer and fill it with the loaded vertex data
; 158  : 	m_NumVertices = subMesh.numVertices;

	mov	eax, DWORD PTR _this$[ebp]
	mov	ecx, DWORD PTR _subMesh$[ebp+8]
	mov	DWORD PTR [eax+120], ecx

; 159  : 	D3D10_BUFFER_DESC bufferDesc;
; 160  : 	bufferDesc.BindFlags = D3D10_BIND_VERTEX_BUFFER;

	mov	DWORD PTR _bufferDesc$[ebp+8], 1

; 161  : 	bufferDesc.Usage = D3D10_USAGE_DEFAULT; // Not a dynamic buffer

	mov	DWORD PTR _bufferDesc$[ebp+4], 0

; 162  : 	bufferDesc.ByteWidth = m_NumVertices * m_VertexSize; // Buffer size
...
; 163  : 	bufferDesc.CPUAccessFlags = 0;   // Indicates that CPU won't access this buffer at all after creation

	mov	DWORD PTR _bufferDesc$[ebp+12], 0

; 164  : 	bufferDesc.MiscFlags = 0;

	mov	DWORD PTR _bufferDesc$[ebp+16], 0

; 165  : 	D3D10_SUBRESOURCE_DATA initData; // Initial data
; 166  : 	initData.pSysMem = subMesh.vertices;   

	mov	eax, DWORD PTR _subMesh$[ebp+12]
	mov	DWORD PTR _initData$[ebp], eax

; 167  : 	if (FAILED( g_pd3dDevice->CreateBuffer( &bufferDesc, &initData, &m_VertexBuffer )))
...
; 171  : 
; 172  : 
; 173  : 	// Create the index buffer - assuming 2-byte (WORD) index data
; 174  : 	m_NumIndices = static_cast<unsigned int>(subMesh.numFaces) * 3;
...
; 175  : 	bufferDesc.BindFlags = D3D10_BIND_INDEX_BUFFER;

	mov	DWORD PTR _bufferDesc$[ebp+8], 2

; 176  : 	bufferDesc.Usage = D3D10_USAGE_DEFAULT;

	mov	DWORD PTR _bufferDesc$[ebp+4], 0

; 177  : 	bufferDesc.ByteWidth = m_NumIndices * sizeof(WORD);
...
; 178  : 	bufferDesc.CPUAccessFlags = 0;

	mov	DWORD PTR _bufferDesc$[ebp+12], 0

; 179  : 	bufferDesc.MiscFlags = 0;

	mov	DWORD PTR _bufferDesc$[ebp+16], 0

; 180  : 	initData.pSysMem = subMesh.faces;   

	mov	eax, DWORD PTR _subMesh$[ebp+32]
	mov	DWORD PTR _initData$[ebp], eax

; 181  : 	if (FAILED( g_pd3dDevice->CreateBuffer( &bufferDesc, &initData, &m_IndexBuffer )))
...
; 46   : 	SAFE_RELEASE( m_IndexBuffer );  // Using a DirectX helper macro to simplify code here - look it up in Defines.h
...
; 47   : 	SAFE_RELEASE( m_VertexBuffer );
...
; 48   : 	SAFE_RELEASE( m_VertexLayout );
...
; 23   : 
; 24   : 	// Good practice to ensure all private data is sensibly initialised
; 25   : 	m_VertexBuffer = NULL;
...
; 28   : 	m_VertexLayout = NULL;

	mov	eax, DWORD PTR _this$[ebp]
	mov	DWORD PTR [eax+1916], 0

; 29   : 
; 30   : 	m_IndexBuffer = NULL;
